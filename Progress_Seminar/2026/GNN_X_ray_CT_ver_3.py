...
def plot_final_results(history, d_true, d_pred, c_true, c_pred, x_cls, c_cls):
    ...
    plt.subplot(1, 2, 1)
    sns.heatmap(confusion_matrix(d_true, np.argmax(d_pred, 1)), annot=True, fmt='g', cmap='Blues', xticklabels=x_cls,
                yticklabels=x_cls)
    plt.title('X-ray Confusion Matrix')
    plt.subplot(1, 2, 2)
    sns.heatmap(confusion_matrix(c_true, np.argmax(c_pred, 1)), annot=True, fmt='g', cmap='Oranges', xticklabels=c_cls,
                yticklabels=c_cls)
    ...
    plt.xlabel('Cancer Predicted');
    plt.ylabel('X-ray Predicted')
    plt.show()
...
def main():
    ...
    c_dir = '/content/drive/MyDrive/Kaggel_direct_download/Cancer_Dataset'

    (xtp, ctp, xtl, ctl), (xvp, cvp, xvl, cvl), x_cls, c_cls = get_final_aligned_data(x_dir, c_dir, 200, 50)
