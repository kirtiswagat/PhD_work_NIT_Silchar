...
class CheXpertDataset(Dataset):
    def __init__(self, csv_path, image_root, max_samples_per_class=1000):
        full_df = pd.read_csv(csv_path)
        self.image_root = image_root

        sampled_dfs = []
        for label in CHEXPERT_LABELS:
            positive_samples = full_df[full_df[label].isin([1.0, -1.0])]
            if len(positive_samples) > max_samples_per_class:
                sampled_dfs.append(positive_samples.sample(n=max_samples_per_class, random_state=42))
            else:
                sampled_dfs.append(positive_samples)

        # Combine sampled dataframes and remove duplicates based on 'Path'
        self.df = pd.concat(sampled_dfs).drop_duplicates(subset=['Path']).reset_index(drop=True)
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(self.image_root + row["Path"]).convert("RGB")

        # Ensure labels are numeric before converting to tensor
        labels = row[CHEXPERT_LABELS].replace(-1, 0).fillna(0).astype(float).values
...
# ============================================================
# def main():
#     train_ds = CheXpertDataset("/content/drive/MyDrive/Kaggel_direct_download/CheXpert-v1.0-small/train.csv", "/content/drive/MyDrive/Kaggel_direct_download/")
#     val_ds = CheXpertDataset("/content/drive/MyDrive/Kaggel_direct_download/CheXpert-v1.0-small/valid.csv", "/content/drive/MyDrive/Kaggel_direct_download/")
#
...
#     optimizer = torch.optim.Adam(model.parameters(), lr=LR)
#
#     history = {k: [] for k in ["train_loss","val_loss","f1","auc"]}
#
